...
export default function StandaloneInvoiceView({
  order,
  settings,
}: {
  order: any;
  ...
  const invoiceNo = `INV-${order.orderNumber?.replace(/[^a-zA-Z0-9]/g, "") || "1001"}`;
  ...
  const items = (order.items || []).map((item: any) => {
    ...
    return {
      ...
      hsn: (settings as any)?.invoiceHsnCode || "84713010",
      qty: item.quantity,
      ...
      <div className="invoice-card max-w-3xl mx-auto bg-white p-4 rounded-xl border border-slate-200 shadow-lg print:shadow-none print:border-none print:rounded-none print:max-w-none print:w-full print:p-0 space-y-2 text-[10px] leading-tight">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 border-b border-slate-300 pb-2">
          ...
          <div className="text-left sm:text-right space-y-0.5 sm:shrink-0 bg-slate-50 sm:bg-transparent p-1.5 sm:p-0 rounded-lg border sm:border-0 border-slate-200">
            <span className="inline-block px-2 py-0.5 rounded bg-blue-100 text-blue-900 font-black text-[9px] uppercase tracking-wider">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 bg-slate-50 p-2 rounded-lg border border-slate-200">
          ...
          <div className="sm:text-right space-y-0.5">
            ...
            <p className="text-[9px] text-slate-700 font-semibold">
              Order Ref: <span className="font-mono font-bold text-slate-900">{order.orderNumber}</span>
            </p>
            <p className="text-[9px] text-slate-700 font-semibold">
          ...
          <table className="w-full text-left text-[9px] border-collapse">
            <thead>
              <tr className="bg-slate-100 text-slate-700 font-bold border-b border-slate-200 uppercase text-[8px]">
                ...
                <th className="py-1 px-2">Item Description & Warranty</th>
                <th className="py-1 px-2 text-center">HSN/SAC</th>
              ...
              {items.map((item: any, idx: number) => (
                <tr key={idx} className="hover:bg-slate-50/50">
                  ...
                  <td className="py-1 px-2">
                    <p className="font-bold text-slate-950 leading-tight">{item.name}</p>
                    <p className="text-[8px] text-emerald-700 font-semibold flex items-center gap-1">
                      <ShieldCheck className="w-2.5 h-2.5" />
                      <span>Warranty: {item.warranty}</span>
                    </p>
                  </td>
                  <td className="py-1 px-2 text-center font-mono text-slate-600">{item.hsn}</td>
                  <td className="py-1 px-2 text-center font-bold text-slate-900">{item.qty}</td>
                  <td className="py-1 px-2 text-right font-mono text-slate-700">₹{item.taxable.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</td>
                  <td className="py-1 px-2 text-right font-mono text-slate-600">₹{item.cgst.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</td>
                  <td className="py-1 px-2 text-right font-mono text-slate-600">₹{item.sgst.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</td>
                  <td className="py-1 px-2 text-right font-mono font-black text-slate-950">₹{item.total.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 pt-1 border-t border-slate-200">
          ...
          <div className="flex flex-col justify-between items-center sm:items-end text-center sm:text-right pt-1 sm:pt-0">
            ...
            <p className="text-[7.5px] text-slate-500 font-mono">Computer Generated Invoice</p>
          </div>
        </div>
      </div>
    </div>
